...
void loop() 
{
  if(Serial.available())
  {
    ...
    if(sig == 1)
    {
      myservo.write(70);   // Close Servo
    }
    else if(sig == 0)
    {
      myservo.write(100);
      Serial.println("Released");     // Open Servo
      motorstop();
      Serial.println("Stop");         // Motor stops after servo open
      while(1);
    }
  }
  if(millis() - t1 >= failsave_time)
    { //fail safe after 2 second
      motorstop();
    }
}
